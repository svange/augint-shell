...
Run `uv run ai-tools workspace status --json $ARGUMENTS`.
...
If this subcommand is unavailable, state that the installed `ai-tools` version is missing `workspace status` and ask the user to upgrade `ai-tools`.
